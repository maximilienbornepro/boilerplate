...
export function CVListPage({ onEdit, onAdapt, onAdaptations }: CVListPageProps) {
  ...
  // Rename modal
  const [renamingCv, setRenamingCv] = useState<CVListItem | null>(null);
  ...
  const handleRenameClick = (e: React.MouseEvent, cv: CVListItem) => {
    e.stopPropagation();
    setRenamingCv(cv);
    setRenameName(cv.name);
  };

  const handleRenameSave = async () => {
    if (!renamingCv || !renameName.trim()) return;
    setRenaming(true);
    try {
      const updated = await api.updateCV(renamingCv.id, { name: renameName.trim() });
      setCvs(prev => prev.map(c => c.id === updated.id ? { ...c, name: updated.name } : c));
      setRenamingCv(null);
      addToast({ type: 'success', message: 'CV renommé' });
    } catch (err: any) {
      addToast({ type: 'error', message: err.message || 'Erreur lors du renommage' });
    } finally {
      setRenaming(false);
    }
  };
  ...
  return (
    <>
      ...
      {/* Rename modal */}
      {renamingCv && (
        <Modal title="Renommer le CV" onClose={() => setRenamingCv(null)}>
          <div className="cv-list-modal-body">
            <FormField label="Nom du CV" required>
              <input
                type="text"
                ...
                onKeyDown={e => {
                  ...
                  if (e.key === 'Escape') setRenamingCv(null);
                }}
                autoFocus
              />
            </FormField>
            <div className="cv-list-modal-actions">
              <Button variant="secondary" onClick={() => setRenamingCv(null)}>
                Annuler
              </Button>
              <Button variant="primary" onClick={handleRenameSave} disabled={!renameName.trim() || renaming}>
                {renaming ? 'Modification...' : 'Modifier'}
              </Button>
            </div>
          </div>
        </Modal>

          ...
          >
            ...
            {siblingBoards.length > 0 && (
              <div ref={boardSwitcherRef} className="delivery-actions-dropdown">
                <button
                  type="button"
                  className="module-header-btn delivery-board-switcher-btn"
                  onClick={() => setShowBoardSwitcher(v => !v)}
                  title={`Board actif : ${board.name} — clique pour basculer`}
                >
                  <span className="delivery-board-switcher-label">{board.name}</span>

...
function todayFrFr(): string {
  // DD/MM/YYYY — still passed to the append-situation skill on the
  // `today` field for backward compat. The skill no longer prints a
  // date header — the field is purely informational now.
...
async function tier2PlaceReviews(base: TierBase & {
  ...
  //
  // `source` (kind + title) sits at the very top so it always survives a
  // tail truncation. The prompt teaches the model to mine the source
  // title for routing hints — meeting names like "Hebdo SFR / Orange"
  // or "Daily TV 17/04" carry strong signal about the right review.
  const ctx = {
    source: {
